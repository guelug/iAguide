...
export default function Visual() {
  const t = useCopy({
    ...
    es: {
      ...
      "embedded": "embebido",
      "agent_rpc": "RPC del agente",
      "codex_native": "Codex nativo",
      "acp_spawn": "spawn ACP",
  ...
  const OPTIONS = [
    { value: "a" as const, label: t.embedded, tone: "var(--teal)" },
    { value: "b" as const, label: t.agent_rpc, tone: "var(--teal)" },
    { value: "c" as const, label: t.codex_native, tone: "var(--violet)" },
    { value: "d" as const, label: t.acp_spawn, tone: "var(--amber)" },
    { value: "e" as const, label: "writer claim", tone: "var(--amber)" },
  ];
  const [step, setStep] = useState<Step>("a");

  return (
    <Figure
      label="OpenClaw: embedded vs external harness"
      ...
      controls={
        <Switcher
          ariaLabel="openclaw-runtime diagram steps"
          value={step}
          onChange={setStep}
          options={OPTIONS}
        />
...
function Scene({ active, t }: { active: Step; t: Record<string, string> }) {
  return (
    <group>
      ...
      />
      <Tag position={[-2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.embedded}</Tag>
      ...
      />
      <Tag position={[-0.75, 1.4500000000000002, 0.0]} tone="amber" center>{t.agent_rpc}</Tag>
      ...
      />
      <Tag position={[0.75, 1.4500000000000002, 0.0]} tone="violet" center>{t.codex_native}</Tag>
      ...
      />
      <Tag position={[2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.acp_spawn}</Tag>
