...
export default function Visual() {
  ...
  const OPTIONS = [
    { value: "a" as const, label: "supervisor", tone: "var(--teal)" },
    { value: "b" as const, label: t.dialogs, tone: "var(--amber)" },
    { value: "c" as const, label: "OOPIF", tone: "var(--violet)" },
    { value: "d" as const, label: t.snapshot, tone: "var(--teal)" },
  ...
  return (
    <Figure
      ...
      controls={
        <Switcher
          ariaLabel="hermes-browser diagram steps"
          value={step}
          onChange={setStep}
          options={OPTIONS}
        />
...
function Scene({ active, t }: { active: Step; t: any }) {
  return (
    <group>
      ...
      />
      <Tag position={[-0.75, 1.4500000000000002, 0.0]} tone="amber" center>{t.dialogs}</Tag>
      ...
      />
      <Tag position={[2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.snapshot}</Tag>
