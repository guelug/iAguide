...
export default function Visual() {
  ...
  const OPTIONS = [
    { value: "a" as const, label: "daemon", tone: "var(--teal)" },
    { value: "b" as const, label: ":18789", tone: "var(--teal)" },
    { value: "c" as const, label: t.clients, tone: "var(--amber)" },
    { value: "d" as const, label: t.nodes, tone: "var(--violet)" },
    { value: "e" as const, label: t.one_per_host, tone: "var(--amber)" },
  ];
  const [step, setStep] = useState<Step>("a");

  return (
    <Figure
      label="OpenClaw: one Gateway daemon"
      ...
      controls={
        <Switcher
          ariaLabel="openclaw-gateway diagram steps"
          value={step}
          onChange={setStep}
          options={OPTIONS}
        />
...
function Scene({ active, t }: { active: Step; t: Record<string, string> }) {
  return (
    <group>
      ...
      />
      <Tag position={[0.75, 1.4500000000000002, 0.0]} tone="violet" center>{t.clients}</Tag>
      ...
      />
      <Tag position={[2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.nodes}</Tag>
      ...
      />
      <Tag position={[0.0, -0.5, 0.0]} tone="amber" center>{t.one_per_host}</Tag>
    </group>
  );
}
