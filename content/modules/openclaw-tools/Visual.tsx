...
export default function Visual() {
  ...
  const OPTIONS = [
    { value: "a" as const, label: t.builtin, tone: "var(--teal)" },
    { value: "b" as const, label: t.groups, tone: "var(--amber)" },
    { value: "c" as const, label: t.plugins, tone: "var(--violet)" },
    { value: "d" as const, label: t.skills, tone: "var(--teal)" },
    { value: "e" as const, label: t.policy, tone: "var(--amber)" },
  ];
  const [step, setStep] = useState<Step>("a");

  return (
    <Figure
      label="OpenClaw: tools, groups, plugins"
      ...
      controls={
        <Switcher
          ariaLabel="openclaw-tools diagram steps"
          value={step}
          onChange={setStep}
          options={OPTIONS}
        />
...
function Scene({ active, t }: { active: Step; t: any }) {
  return (
    <group>
      ...
      />
      <Tag position={[-2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.builtin}</Tag>
      ...
      />
      <Tag position={[-0.75, 1.4500000000000002, 0.0]} tone="amber" center>{t.groups}</Tag>
      ...
      />
      <Tag position={[0.75, 1.4500000000000002, 0.0]} tone="violet" center>{t.plugins}</Tag>
      ...
      />
      <Tag position={[2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.skills}</Tag>
      ...
      />
      <Tag position={[0.0, -0.5, 0.0]} tone="amber" center>{t.policy}</Tag>
    </group>
  );
}
