...
export default function Visual() {
  ...
  const OPTIONS = [
    { value: "a" as const, label: "SKILL.md", tone: "var(--teal)" },
    { value: "b" as const, label: t.gates, tone: "var(--amber)" },
    { value: "c" as const, label: "skill_view", tone: "var(--teal)" },
    { value: "d" as const, label: t.blueprint, tone: "var(--violet)" },
  ...
  return (
    <Figure
      label="Hermes: SKILL.md and blueprints"
      ...
      controls={
        <Switcher
          ariaLabel="hermes-skills diagram steps"
          value={step}
          onChange={setStep}
          options={OPTIONS}
        />
...
function Scene({ active, t }: { active: Step; t: Record<string, string> }) {
  return (
    <group>
      ...
      />
      <Tag position={[-0.75, 1.4500000000000002, 0.0]} tone="amber" center>{t.gates}</Tag>
      ...
      />
      <Tag position={[2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.blueprint}</Tag>
