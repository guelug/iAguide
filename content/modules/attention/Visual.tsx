              ...
              return (
                <group key={h}>
                  {Array.from({ length: 7 }).map((_, i) => {
                    const x = -2.4 + i * 0.8;
                    return (
                      <Node3D key={i} position={[x, y, 0]} color={color} radius={0.08} matte />
                    );
                  })}
          ...
          <>
            {[0, 1, 2, 3].map((h) => {
              ...
              return (
                <group key={h}>
                  {Array.from({ length: 7 }).map((_, i) => (
                    <Node3D key={i} position={[-2.4 + i * 0.8, y, 0]} color={P.amber} radius={0.08} matte />
                  ))}
            ...
            {Array.from({ length: 7 }).map((_, i) => {
              ...
              return (
                <group key={i}>
                  <Node3D position={[x, -1.5, 0]} color={P.violet} radius={0.12} matte pulse={(i % 2) * 0.4} />
                </group>
              );
            })}
            ...
            {Array.from({ length: 7 }).map((_, i) => {
              const x = -2.4 + i * 0.8;
              return i % 2 === 0 ? <Wire key={i} points={[[x, 1.0, 0], [x, -1.4, 0]]} color={P.violet} dashed /> : null;
            })}
            {Array.from({ length: 7 }).map((_, i) => {
              const x = -2.4 + i * 0.8;
              return i % 2 === 1 ? <Wire key={i} points={[[x, 1.0, 0], [x, -1.4, 0]]} color={P.violet} dashed /> : null;
            })}
          </>
        ) : null}
      </Stage>
