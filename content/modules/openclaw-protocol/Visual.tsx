...
export default function Visual() {
  ...
  const OPTIONS = [
    { value: "a" as const, label: t.connect, tone: "var(--teal)" },
    { value: "b" as const, label: "req/res", tone: "var(--teal)" },
    { value: "c" as const, label: t.event, tone: "var(--violet)" },
    { value: "d" as const, label: t.challenge, tone: "var(--amber)" },
    { value: "e" as const, label: "idempotency", tone: "var(--amber)" },
  ];
  const [step, setStep] = useState<Step>("a");

  return (
    <Figure
      label="OpenClaw: typed WS protocol"
      ...
      controls={
        <Switcher
          ariaLabel="openclaw-protocol diagram steps"
          value={step}
          onChange={setStep}
          options={OPTIONS}
        />
...
function Scene({ active, t }: { active: Step; t: any }) {
  return (
    <group>
      ...
      />
      <Tag position={[-2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.connect}</Tag>
      ...
      />
      <Tag position={[0.75, 1.4500000000000002, 0.0]} tone="violet" center>{t.event}</Tag>
      ...
      />
      <Tag position={[2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.challenge}</Tag>
