...
export default function Visual() {
  ...
  const OPTIONS = [
    { value: "a" as const, label: t.core, tone: "var(--teal)" },
    { value: "b" as const, label: t.path, tone: "var(--teal)" },
    { value: "c" as const, label: t.guard, tone: "var(--amber)" },
    { value: "d" as const, label: t.compare, tone: "var(--violet)" },
    { value: "e" as const, label: t.case, tone: "var(--amber)" },
  ];
  const [step, setStep] = useState<Step>("a");

  return (
    <Figure
      label="Hermes: platform adapters"
      ...
      controls={
        <Switcher
          ariaLabel="hermes-platforms diagram steps"
          value={step}
          onChange={setStep}
          options={OPTIONS}
        />
...
function Scene({ active, t }: { active: Step; t: any }) {
  return (
    <group>
      ...
      />
      <Tag position={[-2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.core}</Tag>
      ...
      />
      <Tag position={[-0.75, 1.4500000000000002, 0.0]} tone="amber" center>{t.path}</Tag>
      ...
      />
      <Tag position={[0.75, 1.4500000000000002, 0.0]} tone="violet" center>{t.guard}</Tag>
      ...
      />
      <Tag position={[2.3, 1.4500000000000002, 0.0]} tone="teal" center>{t.compare}</Tag>
      ...
      />
      <Tag position={[0.0, -0.5, 0.0]} tone="amber" center>{t.case}</Tag>
    </group>
  );
}
